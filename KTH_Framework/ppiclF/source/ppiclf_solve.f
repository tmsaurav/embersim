...
! Internal: 
! 
      integer*4 i, j,ip,k,ndum, nstage, istage, dp_loc
      real*8 ythrsh1, ythrsh2, enp, alp,dp_p,z_p,vz_p
      real*8 t_frc,t_imp,vz_imp
!
! Output:
!
      logical iout, zone1z,zone2z
!
         ...
         do j=3,ndum-3,PPICLF_LRS
            ...
            if ((z_p.lt.dp_p).and.(vz_p.le.0.0)) then
               ...
               if (abs(z_p - ppiclf_y1(j)) .gt. 1.0e-6) then
                  ...
                  ! Handle division by zero case (e.g., set t_frc to a default value or skip)
                  t_frc = 0.0
               endif
...
!      do j=3,ndum-3,PPICLF_LRS
!        dp_loc = PPICLF_R_JDP-3+j + ip*(PPICLF_LRP-PPICLF_LRS)
!        ythrsh1 = 0.55*ppiclf_rprop(dp_loc,1)
!        ythrsh2 = 5.0*ppiclf_rprop(dp_loc,1)
!        zone1z = ppiclf_y(j,1).le.ythrsh2 .and. ppiclf_y(j,1).ge.ythrsh1
!        ip = ip + 1
!        k = j + 3       
!     
!        if (zone1z.and.(ppiclf_y(k,1).le.0.0)) then
!          ppiclf_y(k,1) = ppiclf_y(k,1) - (1.0+enp)*ppiclf_y(k,1)
!        endif
!        if ((ppiclf_y(j,1).lt.ythrsh1).and.(ppiclf_y(k,1).le.0.0)) then
!          ppiclf_y(j,1) = ythrsh1
!          ppiclf_y(k,1) = ppiclf_y(k,1) - (1.0+enp)*ppiclf_y(k,1)
!        endif
!      enddo
...
!
     ...
     >          iperiodicz,ndim, i, isl, isr, j, jchk, ic, per
!
