...
m = 3 # no. of rows of buildings(y-dir)
n = 3 # no. of columns of buildings (x-dir)

clx = 15.0 # length of 'cube'
cly = 15.0 # width of 'cube'
clz = 8.0 # height of 'cube'

cnx = 2     + 1     # cube x-dir elements + 1
cny = 2     + 1     # cube y-dir elements + 1
cnz = 2     + 1     # cube height elements + 1
cnout = 2   # elements out from cube in expansion surfaces
stretch_wall = 1.25;   # stretching near ground
...
can_apz_ratio = [apz/lx,1] # ratio for extrusion
nx = [6,20] # no. of macro elements in the apz vs. canopy
nz = [6] # no. of macro elements in the z-dir above cubes - multiple numbers denote numel in each layer
...
refOffsetx = 7.5 #clx/4.0 # size of refinement zone in x-dir
refOffsety = 7.5 # cly/2.0 # size of refinement zone in y-dir
refOffsetz = clz/1.0 # size of refinement zone in z-dir
...
for i in range(0,n):
    for j in range(0,m):
        for k in range(len(vols)):
            ...
            gmsh.model.geo.translate(tmp_dimtag, i*(2*refOffsetx+clx), j*(2*refOffsety+cly), 0)
...
for i in range(len(cubefrsurfs)):
    ...
    canvols.append(gmsh.model.geo.extrude([(2, cubefrsurfs[i])], -lx+refOffsetx, 0, 0, nx, can_apz_ratio, True))
...
#     f.write('APZ stream: dx_apz_min: ' + str((apz-refOffsetx)/nx[0]) + '\n')
#     f.write('Canopy stream: dx_can_min: ' + str(canopy/nx[1]) + '\n')
#     f.write('Wake stream: dx_wake_min: ' + str((wakelx-refOffsetx)/wakenx[0]) + '\n')
...
#     f.write('Refinement stream: dx_ref_min: ' + str((refOffsetx)/(cnout-1)) + '\n')
#     f.write('Refinement span: dy_ref_min: ' + str((refOffsety)/(cnout-1)) + '\n')
#     f.write('Refinement top: dztop_ref_min: ' + str((refOffsetz)/(cnout-1)) + '\n')
